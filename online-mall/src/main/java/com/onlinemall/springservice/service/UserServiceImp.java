...
public class UserServiceImp implements IUserService {
    ...
    public BaseResult<OnlinemallUser> addUser(RequestParams<OnlinemallUser> params) {
        ...
        if(StringUtils.isNotBlank((String)params.getEqual().get(PHONENUMBER))) {
            criteria.andPhonenumberEqualTo((String) params.getEqual().get(PHONENUMBER));
        }
        if(StringUtils.isNotBlank((String)params.getEqual().get(MAIL))){
            criteria.andMailEqualTo((String)params.getEqual().get(MAIL));
        }
        //查询数据
